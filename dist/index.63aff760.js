...
async function getWeather(cityName) {
    try {
        ...
        if (chekBase.cod >= 300) throw new Error(chekBase.message);
        else render(chekBase);
    } catch  {
...
function render(chekBase) {
    ...
    let sunrise = (0, _dateFns.format)(new Date(chekBase.sys.sunrise) * 1000, "HH:mm");
    let sunset = (0, _dateFns.format)(new Date(chekBase.sys.sunset) * 1000, "HH:mm");
...
function renderFavorites() {
    ...
    if (localStorage.getItem("lastCity")) getWeather(lastCity);
    else getWeather("Moscow");
}
function showFromFavorite(event, lastCity) {
    let city = event.target.textContent;
    getWeather(city);
    (0, _localJs.lastFavoriteViewed)(city);
    console.log(city);
...
function addCity() {
    ...
    if (city !== "---") {
        let position = cities.findIndex((anyCity)=>anyCity == city);
        if (position >= 0) cities.splice(position, 1);
        else cities.push(city);
    } else throw new Error("Отсутствует город для добавления");
    (0, _localJs.toStorage)(cities);
    cities = JSON.parse(localStorage.getItem("citiesArray"));
    renderFavorites();
}
let cities;
let lastCity;
if (localStorage.getItem("citiesArray")) cities = JSON.parse(localStorage.getItem("citiesArray"));
else cities = [];
document.querySelector("#favorite_button").addEventListener("click", addCity);
renderFavorites();

},{"./local.js":"diKbX","date-fns":"9yHCA"}],"diKbX":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "toStorage", ()=>toStorage);
parcelHelpers.export(exports, "lastFavoriteViewed", ()=>lastFavoriteViewed);
function toStorage(cities) {
    let citiesArray = JSON.stringify(cities);
    localStorage.setItem("citiesArray", citiesArray);
}
function lastFavoriteViewed(city) {
    let lastCity = city;
...
},{"../toDate/index.js":"fsust","../_lib/toInteger/index.js":"f7kKX","../_lib/requiredArgs/index.js":"9wUgQ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"iOhcx":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["1JEHZ","adjPd"], "adjPd", "parcelRequire8fb7")
